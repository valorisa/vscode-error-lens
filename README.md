...
|editorHoverEnabled|**true**|When checked - shows tooltip when hovering over diagnostic with buttons to (exclude problem, go to rule documentation, go to rule definition(linter file)...).|
...
|lintFilePaths|\{"eslint":\["\*\*/\*.eslintrc.\{js,cjs,yaml,yml,json\}","\*\*/\*package.json"\],"Stylelint":\["\*\*/\*.stylelintrc","\*\*/\*.stylelintrc.\{cjs,js,json,yaml,yml\}","\*\*/\*stylelint.config.\{cjs,js\}","\*\*/\*package.json"\]\}|Specify where to search for linter rule definitions by diagnostic source ([glob](https://code.visualstudio.com/api/references/vscode-api#GlobPattern) for local linter files). `node_modules` folder is excluded.|
